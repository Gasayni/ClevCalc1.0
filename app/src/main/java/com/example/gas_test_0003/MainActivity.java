package com.example.gas_test_0003;
...
public class MainActivity extends AppCompatActivity implements SeekBar.OnSeekBarChangeListener {
    ...
    public void onButtonClickPlus(View v) {
        ...
        // помещаем в e1 из нумерного поля Num1
        /*EditText e1 = (EditText)findViewById(R.id.Num1);
        EditText e2 = (EditText)findViewById(R.id.Num2);
        // помещаем в resText из текстового поля Result
        TextView resText = (TextView)findViewById(R.id.Result);// мы взяли конкретные поля по их id и записываем их в переменные
//
        int a = Integer.parseInt(e1.getText().toString());
        int b = Integer.parseInt(e2.getText().toString());
        int sum = a+b;

        // помещаем в resText(поле) текст
        // в текстовое поле нам нужно поместить именно строку
...
//                            System.out.println("Убираем уже ненужные скобки ");
    ...
    public void onButtonClick00(View v) {
        TextView resText = (TextView) findViewById(R.id.Result);
        String OldTextString = resText.getText().toString(); // перевод из текстового формата в String

        String[] s = OldTextString.split("");
        if (OldTextString.equals("0")) {
        } else if (OldTextString.equals("")) {
            resText.setText(OldTextString + 0);
        } else if (s[s.length - 1].equals(")")) {
            resText.setText(OldTextString + "*" + "0");
        } else resText.setText(OldTextString + 0 + 0);
